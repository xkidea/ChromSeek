...
class MultiOmicsModel_2Mb(nn.Module):
    ...
    def _load_pretrained(self, pretrained_path):
        ...
        new_state = {}
        prefix_candidates = ("", "dna_encoder.encoder_model.")
        for k, v in state_dict.items():
            clean_k = k.replace('module.', '')
            for prefix in prefix_candidates:
                new_k = clean_k
                if prefix and clean_k.startswith(prefix):
                    new_k = clean_k[len(prefix):]
...
class MultiOmicsPredictor(nn.Module):
    def __init__(
        ...
        if pretrained_path:
            self._load_backbone_pretrained(pretrained_path)

    def _load_backbone_pretrained(self, pretrained_path: str) -> None:
        state_dict = torch.load(pretrained_path, map_location="cpu", weights_only=True)
        my_state = self.state_dict()
        allowed_prefixes = (
            "dna_encoder",
        ...
        for k, v in state_dict.items():
            clean_k = k.replace("module.", "")
            if not any(clean_k == prefix or clean_k.startswith(f"{prefix}.") for prefix in allowed_prefixes):
                continue
            if clean_k in my_state and my_state[clean_k].shape == v.shape:
                new_state[clean_k] = v
        if new_state:
            self.load_state_dict(new_state, strict=False)
