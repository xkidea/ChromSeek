...
def main():
    model_path = PROJECT_ROOT / "checkpoints" / "transfer_multiomics_best.pth"
    data_path = TASK_DIR / "sample_data.pt"
    if not os.path.exists(model_path) or not os.path.exists(data_path):
        print("Required files are missing. Please make sure checkpoints/transfer_multiomics_best.pth and sample_data.pt exist.")
        return
    ...
    output_path = TASK_DIR / "multiomics_prediction_example.png"
    plt.savefig(output_path, dpi=150)
    print(f"Saved prediction example to {output_path}")
